...
Map the active include chain, modular macros, screen helpers, and retained snapshots
before planning a change. Tie motion, sensorless homing, probing, heater, and motor
assumptions to this K1 Max. Do not reuse Ender Cartesian calibration as a default.

## Acceptance evidence

Review include targets, duplicate sections, macro references, units, motion/thermal
bounds, and recovery ordering. State which evidence is static and which requires an
operator-controlled test on the actual printer.
...
No repository check proves hardware safety. Preserve known-good configurations and do
not inspect private database contents. Connecting, flashing, restarting, heating,
homing, and movement require separate scoped authorization and rollback.
